...
class Communicator(object):
    ...
    def myAllreduce(self, src_array, dest_array, op=MPI.SUM):
        ...
        def reduction(op: MPI.Op, target_arr: npt.NDArray, source_arr: npt.NDArray) -> None:
            if op==MPI.SUM:
                np.copyto(target_arr, target_arr+source_arr)
            elif op==MPI.MAX:
                np.copyto(target_arr, np.maximum(target_arr, source_arr))
            elif op==MPI.MIN:
                np.copyto(target_arr, np.minimum(target_arr, source_arr))
        ...
        if rank!=0:
            ...
            curproc_array = np.empty_like(dest_array)
            np.copyto(dest_array, src_array)
            for curproc_rank in range(1, nprocs):
                self.comm.Recv(curproc_array, curproc_rank)
                reduction(op, dest_array, curproc_array)
        ...
        if rank!=0:
            ...
            for curproc_rank in range(1, nprocs):
                self.comm.Send(dest_array, curproc_rank)
    ...
    def myAlltoall(self, src_array, dest_array):
        ...
        chunk_size = src_array.size // size
        
        for i in range(size):
            send_chunk = src_array[i * chunk_size: (i + 1) * chunk_size]
            recv_chunk = np.empty_like(send_chunk)
            
            if i == rank:
                np.copyto(dest_array[i * chunk_size: (i + 1) * chunk_size], send_chunk)
            else:
                self.comm.Sendrecv(send_chunk, dest=i, sendtag=0,
                                recvbuf=recv_chunk, source=i, recvtag=0)
                dest_array[i * chunk_size: (i + 1) * chunk_size] = recv_chunk
        seg_bytes = chunk_size * src_array.itemsize
        self.total_bytes_transferred += seg_bytes * (size - 1) * 2
